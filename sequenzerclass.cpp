...
void SequenzerClass::SetDemoSong()
{
    ...
    Sounds[0].Attack = 0x01;
    Sounds[0].Decay = 0x00;
    Sounds[0].Sustain = 0x0f;
    Sounds[0].Release = 0x0f;
    Sounds[0].Waveform = 2;
    Sounds[0].Pulsweite = 0x7fff;

    Sounds[1].Attack = 0x01;
    Sounds[1].Decay = 0x00;
    Sounds[1].Sustain = 0x0f;
    Sounds[1].Release = 0x0f;
    Sounds[1].Waveform = 8;
    Sounds[1].Pulsweite = 0x7fff;

    for(int i=0;i<PATTERN_LEN/4;i++)
    {
        Pattern[0].Note[i*4+0]=33;
        Pattern[0].KeyOff[i*4+1] = true;
        Pattern[0].Note[i*4+2]=39;
        Pattern[0].KeyOff[i*4+3] = true;
    }

    for(int i=0;i<PATTERN_LEN/4;i++)
    {
        Pattern[1].Note[i*4+0]=33;
        Pattern[1].SoundNr[i*4+0]=1;
        Pattern[1].KeyOff[i*4+1] = true;
    }
...
void SequenzerClass::PlayTrack(unsigned short pattern_nr, int sid_nr, int voice)
{
    ...
    bool KeyOff = Pattern[pattern_nr].KeyOff[PatternPos];

    //PushSIDStack(sid_nr,24,15);

    if(Note == 0xff)
    {
        if(KeyOff) PushSIDStack(sid_nr,voice*7+4,ShadowIO[sid_nr][voice*7+4] & 0xFE);
        return;
    }
